...
from train_green_armored import collate_fn
import os
import numpy as np
import tqdm
import json
...
class TestAndVal():
    ...
    def predict_image(self, image_path, save_path, prompt='single'):
        ...
        if self.texts == 'single':
            texts = "A vehicle covered in green camouflage"
        else:
    ...
    def apply_camouflage_texture(self, image_name, textures_file):
        ...
        textures_adv = cal_texture__(texture_content_adv)
        dataset.set_textures(textures_adv)
        image_name_ = image_name + '.npz'
        data_dict = dataset.apply(image_name_)['img']
        # save adv image
        img_np = data_dict['inputs'][0].permute(
            1, 2, 0).cpu().numpy().astype(np.uint8)
        cv2.imwrite(
            f'test/appd/ori/{image_name}.png', img_np[:, :, ::-1])
        return data_dict

    def val_asr(self, train_type, textures_file):
        """
        usage
        """
        def cal_texture(texture_content):
        ...
        def cal_texture_green(texture_content):
            min_color = torch.tensor(
                [0.03, 0.08, 0.02], device=texture_content.device)
            max_color = torch.tensor(
                [0.25, 0.45, 0.18], device=texture_content.device)
        ...
        def cal_texture_brown(texture_content):
            # keep in brown color
            min_color = torch.tensor([0.45, 0.40, 0.28], device=texture_content.device)
            max_color = torch.tensor([0.70, 0.65, 0.48], device=texture_content.device)
            textures = 0.5 * (torch.nn.Tanh()(texture_content) + 1)
            textures = textures * (max_color - min_color) + min_color
            textures = torch.clamp(textures, 0.0, 1.0)
            return texture_origin * (1 - texture_mask) + texture_mask * textures

        if "brown" in train_type:
            cal_texture = cal_texture_brown
        elif "green" in train_type:
            cal_texture = cal_texture_green
        else:
            cal_texture = cal_texture
        if "armored" in train_type:
            mask_dir = '/root/autodl-tmp/dataset/val/mask/'
            data_dir = '/root/autodl-tmp/dataset/val/npz/'
            label_dir = '/root/autodl-tmp/dataset/val/label/'
            obj_file = 'data/test.obj'
            vehicle_type = 'armored'
            if "das" in train_type:
                faces_file = "data/selected_face_ids.txt"
            else:
                faces_file = "data/faces_new.txt"
        else:
            mask_dir = '/root/autodl-tmp/data_tgrs2/test/mask/'
            data_dir = '/root/autodl-tmp/data_tgrs2/test/npz/'
            label_dir = '/root/autodl-tmp/data_tgrs2/test/label/'
            obj_file = 'data/etron/audi_et_te.obj'
            vehicle_type = 'car'
            if "das" in train_type:
                faces_file = "data/etron/all_faces.txt"
            else:
                faces_file = "data/etron/exterior_face.txt"
        save_path = train_type.replace('textures', 'results')
        ...
        texture_mask = torch.from_numpy(
            texture_mask).cuda(device=0).unsqueeze(0)
        dataset = DatasetAdv(
            vehicle_type, data_dir, self.texts, image_size, texture_size, faces, vertices, distence=None, mask_dir=mask_dir, ret_mask=True)
        loader = DataLoader(
            dataset=dataset,
            batch_size=1,
            shuffle=False,
        ...
        os.makedirs(f'{save_path}/adv/{model_name}/{model_train}', exist_ok=True)
        pt_save_path = f'{save_path}/adv/{model_name}/{model_train}/{model_name}_{self.texts}_{self.threshold}.pt'
        ...
        dir_name = os.path.basename(train_type)
        os.makedirs(f'test/images_val/{dir_name}', exist_ok=True)
        for i, sample in pbar:
            ...
            if i % 100 == 0:
                ...
                plt.imsave(f"test/test.png", image_numpy)
    ...
    def val_asr_transfer_sentence(self, textures_file):
        """
        usage
        """
        def yolo_to_xyxy(box, img_w, img_h):
        ...
        if self.texts == 'single':
            ...
            self.texts_list = ["A vehicle covered in green camouflage", "An army-green armored vehicle in the clearing", "A green military armored vehicle driving on the road"]
        results_dict = {
            text: {
                'clean': {},
                'adv': {}
        ...
        os.makedirs(f'results_transfer_sen/adv/{model_name}/{model_train}', exist_ok=True)
        pt_save_path = f'results_transfer_sen/adv/{model_name}/{model_train}/{model_name}_{self.texts}_{self.threshold}.pt'
        torch.save(results_dict, pt_save_path)
        print(f'Results have been saved to {pt_save_path}!')
    ...
    def val_asr_transfer(self, textures_file, attack_model):
        """
        usage
        """
        def yolo_to_xyxy(box, img_w, img_h):
        ...
        iou_threshold = 0.3  # select more ious
        model_name = self.auto_model_name(self.config)
        texture_name = self.auto_model_name(textures_file)
        if model_name == 'yolo':
            import sys
            sys.path.append('/root/ovd-attack/YOLO-World')
            model = init_detector(self.config, self.checkpoint)
        ...
        os.makedirs(f'results_transfer/adv/{attack_model}/{texture_name}', exist_ok=True)
        pt_save_path = f'results_transfer/adv/{attack_model}/{texture_name}/{model_name}_{self.texts}_{self.threshold}.pt'
        torch.save(results_dict, pt_save_path)
        print(f'Results have been saved to {pt_save_path}!')
...
if __name__ == '__main__':
    ...
    argparse.add_argument(
        ...
        '--checkpoint', type=str, default='/root/autodl-tmp/weights/groundingdino/groundingdino_swint_ogc_mmdet-822d7e9d.pth', help='checkpoint file')
    argparse.add_argument(
    ...
    def cal_asr_all_models(train_type):
        models = ['glip', 'dino', 'yolo']
        if "das" in train_type or "fca" in train_type:
            index = 2
            train = "single"
        elif "aug" in train_type:
            index = 2
            train = "augment"
        else:
            index = 5
        ...
        for model in models:
            ...
            for theroshold in therosholds:
                print(f"model: {model},theroshold: {theroshold}")
                args.theroshold = theroshold
                if model == 'glip':
                    args.config = 'mmdetection/configs/glip/glip_atss_swin-t_a_fpn_dyhead_16xb2_ms-2x_funtune_coco.py'
                    args.checkpoint = '/root/autodl-tmp/weights/glip/glip_atss_swin-t_a_fpn_dyhead_16xb2_ms-2x_funtune_coco_20230914_180419-e6addd96.pth'
                    args.textures_file = f'{train_type}/glip/texture_camouflage_glip_{train}_epoch_{index}.npy'
                elif model == 'dino':
                    args.config = 'mmdetection/configs/grounding_dino/grounding_dino_swin-t_pretrain_obj365_goldg_cap4m.py'
                    args.checkpoint = '/root/autodl-tmp/weights/groundingdino/groundingdino_swint_ogc_mmdet-822d7e9d.pth'
                    args.textures_file = f'{train_type}/dino/texture_camouflage_dino_{train}_epoch_{index}.npy'
                else:
                    ...
                    args.checkpoint = '/root/autodl-tmp/weights/yolo_world/yolo_world_v2_l_obj365v1_goldg_pretrain_1280ft-9babe3f6.pth'
                    args.textures_file = f'{train_type}/yolo/texture_camouflage_yolo_{train}_epoch_{index}.npy'
                print(args)
                test = TestAndVal(args.texts_mode, args.theroshold,
                                    args.config, args.checkpoint)
                test.val_asr(train_type, args.textures_file)

    for train_type in ["/root/autodl-tmp/res/tgrs2/textures_car_green_aug"]:
        # das: armored, car; fca: car                             
        cal_asr_all_models(train_type)

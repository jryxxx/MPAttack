...
def get_params(vehicle_type, carlaTcam, carlaTveh):
...
class NeuralRenderer(torch.nn.Module):
    ...
    def __init__(self, img_size=720):
        ...
        # test example
        eye, camera_direction, camera_up = get_params("car",
        ...
        # light
        self.renderer.renderer.light_intensity_ambient = 0.5  # 0.5
        self.renderer.renderer.light_intensity_directional = 0.5  # 0.5
...
def example():
    obj_file = '/root/ovd-attack/data/audi_et_te.obj'
    data_path = '/root/autodl-tmp/dataset/train/npz'
    img_save_dir = '/root/ovd-attack/render_test_res'

    vertices, faces = neural_renderer.load_obj(obj_file)

    texture_mask = np.zeros((faces.shape[0], 2, 2, 2, 3), 'int8')
    with open('/root/ovd-attack/data/exterior_face.txt', 'r') as f:
    ...
    for data_path in data_lsit:
        ...
        imageio.imsave(img_save_dir + 'test_render.png', im_rendered)
        imageio.imsave(img_save_dir + 'test_origin.png', img)
        imageio.imsave(img_save_dir + 'test_total.png',
                       np.add(img, 255 * im_rendered))

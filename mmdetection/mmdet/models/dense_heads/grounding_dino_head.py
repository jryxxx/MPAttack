    ...
    def predict(self,
        ...
        predictions = self.predict_by_feat(
            *outs,
            ...
            rescale=rescale)
        # predictions, losses = self.predict_by_feat(
        #     *outs,
        #     batch_img_metas=batch_img_metas,
        #     batch_token_positive_maps=batch_token_positive_maps,
        #     rescale=rescale)
    ...
    def predict_by_feat(self,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            results = self._predict_by_feat_single(cls_score, bbox_pred,
                                                   token_positive_maps,
                                                   img_meta, rescale)
            # results, losses = self._predict_by_feat_single(cls_score, bbox_pred,
            #                                                token_positive_maps,
            #                                                img_meta, rescale)
    ...
    def _predict_by_feat_single(self,
        ...
        if token_positive_maps is not None:
            ...
            cls_score = convert_grounding_to_cls_scores(
                logits=cls_score.sigmoid()[None],
                positive_maps=[token_positive_maps])[0]
            scores = cls_score.clone().view(-1)

            def get_loss(scores):
                low_score = 0.1
                high_score = 0.4
                penalty_high = torch.relu(scores - low_score)
                penalty_low = torch.relu(high_score - scores)
                mid_scores = scores[(scores >= high_score) & (
                    scores <= low_score)]
                reward = torch.mean(
                    mid_scores) if mid_scores.numel() > 0 else 0
                loss = penalty_high.mean() + penalty_low.mean() - 0.5 * reward
                return loss
            # losses = get_loss(scores)
            scores, indexes = cls_score.view(-1).topk(max_per_img)
            num_classes = cls_score.shape[-1]
            det_labels = indexes % num_classes
            bbox_index = indexes // num_classes

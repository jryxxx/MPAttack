...
def main():
    global args, logger
    args = get_parser('/root/ovd-attack/dataset_cod/cod_mgl50_mgl.yaml')
    check(args)
    logger = get_logger()
    ...
    for image in images:
        args.save_folder = f'/root/ovd-attack/dataset_cod/results_mgl/{image}'
        save_folder = args.save_folder
        check_makedirs(save_folder)
        cod_folder = save_folder
        ...
        args.data_root = f'/root/ovd-attack/test/images_val/{image}'
        if "car" in image:
            args.test_list = f'/root/ovd-attack/dataset_cod/test_car.lst'
        else:
            args.test_list = f'/root/ovd-attack/dataset_cod/test_armored.lst'
        ...
        if not args.has_prediction:
            ...
            test(test_loader, test_data.data_list, model, cod_folder)
        # if args.split != 'test':
        #     calc_acc(test_data.data_list, cod_folder, coee_folder)


def test(test_loader, data_list, model, cod_folder):

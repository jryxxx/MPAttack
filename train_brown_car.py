...
def parse_args():
    ...
    parser.add_argument(
        '--data-path',
        default='/root/autodl-tmp/data_tgrs2/train',
    ...
    parser.add_argument(
        '--obj-file',
        default='data/etron/audi_et_te.obj',
    ...
    parser.add_argument(
        '--faces',
        default='data/etron/exterior_face.txt',
...
def cal_texture(texture_param, texture_origin, texture_mask, device, texture_content=None, content=False):
    # keep in yellow color
    # min_color = torch.tensor([0.65, 0.55, 0.10], device=device)
    # max_color = torch.tensor([0.90, 0.75, 0.25], device=device)
    min_color = torch.tensor([0.45, 0.40, 0.28], device=device)  # 深、灰、棕基调
    max_color = torch.tensor([0.70, 0.65, 0.48], device=device)  # 哑光沙棕亮度上限
...
def main(args):
    ...
    output_path = "/root/autodl-tmp/res/tgrs2"
    if not os.path.exists(output_path):
        os.makedirs(output_path)
    if not os.path.exists(os.path.join(output_path, 'textures_car_brown')):
        os.makedirs(os.path.join(output_path, 'textures_car_brown'))
    if not os.path.exists(os.path.join(output_path, 'results_car_brown')):
        os.makedirs(os.path.join(output_path, 'results_car_brown'))
    ...
    ret_mask = True
    dataset = DatasetAdv(
        "car", data_dir, args.prompt_mode, img_size, texture_size, faces, vertices, distence=None, mask_dir=mask_dir, ret_mask=ret_mask)
    ...
    for epoch in range(1, args.epoches + 1):
        ...
        os.makedirs(f'{output_path}/textures_car_brown/{model_type}', exist_ok=True)
        os.makedirs(f'{output_path}/results_car_brown/loss/{model_type}', exist_ok=True)
        np.save(f"{output_path}/textures_car_brown/{model_type}/texture_camouflage_{model_type}_{args.prompt_mode}_epoch_{epoch}.npy",
                texture_param.data.cpu().numpy())
        loss_tensor = torch.stack(loss_list)
        torch.save(
            loss_tensor, f'{output_path}/results_car_brown/loss/{model_type}/{model_type}_{args.prompt_mode}_losses.pt')

...
def main(args):
    ...
    if not os.path.exists(os.path.join(output_path, 'textures_armored_brown')):
        os.makedirs(os.path.join(output_path, 'textures_armored_brown'))
    if not os.path.exists(os.path.join(output_path, 'results_armored_brown')):
        os.makedirs(os.path.join(output_path, 'results_armored_brown'))
    ...
    ret_mask = True
    dataset = DatasetAdv(
        "armored", data_dir, args.prompt_mode, img_size, texture_size, faces, vertices, distence=None, mask_dir=mask_dir, ret_mask=ret_mask)
    ...
    for epoch in range(1, args.epoches + 1):
        ...
        os.makedirs(f'{output_path}/textures_armored_brown/{model_type}', exist_ok=True)
        os.makedirs(f'{output_path}/results_armored_brown/loss/{model_type}', exist_ok=True)
        np.save(f"{output_path}/textures_armored_brown/{model_type}/texture_camouflage_{model_type}_{args.prompt_mode}_epoch_{epoch}.npy",
                texture_param.data.cpu().numpy())
        loss_tensor = torch.stack(loss_list)
        torch.save(
            loss_tensor, f'{output_path}/results_armored_brown/loss/{model_type}/{model_type}_{args.prompt_mode}_losses.pt')
